#####################################################################################
## This script calculates p-values on GEOquery Data:
...
### Computing p-values
    ...
    for(i in 1:choose(nsmall, nlow)){
        ...
        signed_pvals_lowdose_ttest_permuted[i,] <- signed_ttest_mat(temp_data,lowdose,control,test_signs)$pvals
    }
    signed_pvals_permutation_test <- colSums(abs(signed_pvals_lowdose_ttest_permuted)-rep(1,choose(nsmall,nlow))%*%t(abs(signed_pvals_lowdose_ttest))<=0)/choose(nsmall,nlow)
    n <- nrow(Data)
    output <- list()
    ## output$pvals=signed_pvals_permutation_test*(1-1/(1+choose(nsmall,nlow)))
    output$pvals <- signed_pvals_permutation_test - 1/choose(nsmall,nlow) * runif(n)
